...
def load_data(train_arr, test_arr,final_embeding):
    
    labels = np.loadtxt("data/MDAD/adj1.txt")
    nd= np.max(labels[:,0])
    nm = np.max(labels[:,1])
    ...
    M = np.loadtxt("data/MDAD/interaction.csv", delimiter=",")
